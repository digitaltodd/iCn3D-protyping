...
class Events {
    ...
    allEventFunctions() { let me = this.icn3dui, ic = me.icn3d;
        ...
        me.myEventCls.onIds("#" + me.pre + "reload_collectionfile", "click", function(e) { me.icn3d;
            e.preventDefault();
            if(!me.cfg.notebook) dialog.dialog( "close" );
            if(!me.cfg.notebook) {
                $(".ui-dialog-content").dialog("close");
            ...
            if(!file) {
                ...
                reader.onload = async function(e) {
                    ...
                    let collection = [dataStr['structures'].map(({filename}) => filename.split('.')[0]), dataStr['structures'].map(({title}) => title)];
                    let collectionHtml = ic.selectCollectionsCls.setAtomMenu(collection[0], collection[1]);
                    $.when(ic.chainalignParserCls.downloadMmdbAf(collection[0].toString())).then(function(){
                        console.log("during")
                    ...
                    me.htmlCls.clickMenuCls.setLogCmd('load collection file ' + $("#" + me.pre + "collectionfile").val(), false);
                };
                reader.readAsText(file);
