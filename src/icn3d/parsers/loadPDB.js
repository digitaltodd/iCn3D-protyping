...
class LoadPDB {
  ...
  getStructureId(id, moleculeNum, bMutation) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let structure = id;

    if (id == ic.defaultPdbId || bMutation) {
  ...
  loadPDB(src, pdbid, bOpm, bVector, bMutation, bAppend, type, bEsmfold) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let hAtoms = {};
      ...
      if (record === "HEADER" && !bHeader && !pdbid) {
        ...
        if (id == "") {
          if (bAppend) {
            ...
            id =
              ic.inputid && ic.inputid.indexOf("/") == -1
                ? ic.inputid.substr(0, 10)
                : ic.defaultPdbId; //ic.filename.substr(0, 4);
          }
        }
        ...
        let name = line
          .substr(10)
          .replace(/ALPHAFOLD MONOMER V2.0 PREDICTION FOR /gi, "");
        ...
        let startChain =
          line.substr(18, 2).trim() == "" ? "A" : line.substr(18, 2).trim();
        ...
        let startChain =
          line.substr(20, 2).trim() == "" ? "A" : line.substr(20, 2).trim();
        ...
        if (line.indexOf("1/2 of bilayer thickness:") !== -1) {
          ...
          ic.halfBilayerSize = parseFloat(
            line.substr(line.indexOf(":") + 1).trim()
          );
        } else if (remarkType == 210) {
          if (
            line.substr(11, 32).trim() == "EXPERIMENT TYPE" &&
            line.substr(45).trim() == "NMR"
          ) {
            bNMR = true;
          }
          ...
          if (ic.biomtMatrices[m] == undefined)
            ic.biomtMatrices[m] = new THREE.Matrix4().identity();
          ...
          ic.biomtMatrices[m].elements[n + 12] = parseFloat(
            line.substr(54, 14)
          );
        }
        // missing residues
          ...
          if (ic.chainMissingResidueArray[chainNum] === undefined)
            ic.chainMissingResidueArray[chainNum] = [];
          let resObject = {};
          resObject.resi = resi;
          ...
          remarkType == 900 &&
          ic.emd === undefined &&
          line.substr(34).trim() == "RELATED DB: EMDB"
        ) {
        ...
        //}
          ...
          !ic.atoms[serial].het &&
          me.parasCls.residueColors.hasOwnProperty(
            ic.atoms[serial].resn.toUpperCase()
          )
        ) {
          secondaries = "c";
        } else {
        ...
        if (oriResidueNum !== prevOriResidueNum) {
          ...
          if (chainNum !== prevChainNum) {
            ...
            if (serial !== 1 && prevChainNum !== "") {
              if (ic.chains[prevChainNum] === undefined)
                ic.chains[prevChainNum] = {};
              ic.chains[prevChainNum] = me.hashUtilsCls.unionHash(
                ic.chains[prevChainNum],
                chainsTmp
              );
            }
            ...
            if (ic.structures[structure.toString()] === undefined)
              ic.structures[structure.toString()] = [];
            if (!ic.structures[structure.toString()].includes(chainNum))
              ic.structures[structure.toString()].push(chainNum);

            if (ic.chainsSeq[chainNum] === undefined)
              ic.chainsSeq[chainNum] = [];
        ...
        for (let j = 0; j < 4; ++j) {
          ...
          if (ic.atoms[oriSerial2NewSerial[from]] !== undefined)
            ic.atoms[oriSerial2NewSerial[from]].bonds.push(
              oriSerial2NewSerial[to]
            );
        }
    ...
    ic.chains[chainNum] = me.hashUtilsCls.unionHash2Atoms(
      ic.chains[chainNum],
      chainsTmp,
      ic.atoms
    );
    ...
    let curChain = firstAtom.chain,
      curResi = firstAtom.resi,
      curInsc,
      curResAtoms = [];
      ...
      if (!atom.het) {
        if ($.inArray(atom.resn, me.parasCls.nucleotidesArray) !== -1) {
          ...
          if (atom.name === "O3'" || atom.name === "O3*") {
            ...
            ic.secondaries[
              atom.structure + "_" + atom.chain + "_" + atom.resi
            ] = "o"; // nucleotide
          }
        ...
        //}
          ...
          $.inArray(atom.resn, me.parasCls.ionsArray) !== -1 ||
          atom.elem.trim() === atom.resn.trim()
        ) {
  ...
  refreshBonds(curResAtoms, prevCarbon) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    ...
    for (let j = 0; j < n; ++j) {
      ...
      for (let k = j + 1; k < n; ++k) {
        ...
        if (
          atom0.alt === atom1.alt &&
          me.utilsCls.hasCovalentBond(atom0, atom1)
        ) {
      ...
      if (
        prevCarbon &&
        (prevCarbon.name === "C" || prevCarbon.name === "O3'") &&
        (atom0.name === "N" || atom0.name === "P") &&
        me.utilsCls.hasCovalentBond(atom0, prevCarbon)
      ) {
  ...
  adjustSeq(chainMissingResidueArray) {
    ...
    for (let chainNum in ic.chainsSeq) {
      ...
      ic.chainsSeq[chainNum] = this.mergeTwoSequences(
  ...
  mergeTwoSequences(A, B) {
    ...
    while (i < m && j < n) {
      let aResi = parseInt(A[i].resi),
        bResi = parseInt(B[j].resi);
  ...
  setSsbond(chainidHash) {
    ...
    for (let chainid in ic.chainsSeq) {
      ...
      for (let i = 0, il = seq.length; i < il; ++i) {
        ...
        if (seq[i].name == "C") {
          if (structure2cys_resid[structure] == undefined)
            structure2cys_resid[structure] = [];
    ...
    for (let chainid in chains) {
      ...
      for (let i = 0, il = serialArray.length; i < il; ++i) {
        ...
        if (
          ...
          (ic.nucleotides.hasOwnProperty(serialArray[i]) &&
            (atom.name == "O3'" || atom.name == "O3*"))
        ) {
          if (atom.resi == lastResi) continue; // e.g., Alt A and B

          let resn =
            atom.resn.trim().length > 3
              ? atom.resn.trim().substr(0, 3)
              : atom.resn.trim();
  ...
  isSecondary(resid, residArray, bNMR) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    if (!bNMR) {
      ...
      for (let i = 0, il = residArray.length; i < il; ++i) {
        if (
          chain_resi == residArray[i].substr(residArray[i].indexOf("_") + 1)
        ) {
          bFound = true;
          break;
        }
