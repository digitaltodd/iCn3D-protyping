...
class MmdbParser {
  ...
  async downloadBlast_rep_id(sequence_structure_ids) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    //ic.bCid = undefined;
  ...
  async downloadRefseq(refseqid, bBlast_rep_id) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let url =
      me.htmlCls.baseUrl + "vastdyn/vastdyn.cgi?refseq2uniprot=" + refseqid;

    me.cfg.refseqid = refseqid;

    //ic.bCid = undefined;

    let data = await me.getAjaxPromise(
      url,
      "jsonp",
      false,
      "The protein accession " +
        refseqid +
        " can not be mapped to AlphaFold UniProt ID..."
    );
    ...
    if (data && data.uniprot) {
      ...
      alert(
        "The accession " +
          refseqid +
          " can not be mapped to AlphaFold UniProt ID. It will be treated as a UniProt ID instead."
      );
  ...
  async parseMmdbData(
    ...
    if (type === undefined) {
      ...
      // multiple mmdbids, typically for alignment
    ...
    if (type === undefined)
      ic.ParserUtilsCls.setYourNote(
        structure.toUpperCase() + "(MMDB) in iCn3D"
      );
    ...
    for (let molid in data.domains) {
      ...
      for (
        let index = 0, indexl = domainArray.length;
        index < indexl;
        ++index
      ) {
        let domainName =
          structure + "_" + chain + "_3d_domain_" + (index + 1).toString();
        ...
        for (let i = 0, il = subdomainArray.length; i < il; ++i) {
          ...
          if (
            domainFromHash.hasOwnProperty(domainFrom) ||
            domainToHash.hasOwnProperty(domainTo)
          ) {
    ...
    if (
      ...
      Object.keys(ic.structures).length == 1
    ) {
      if ($("#" + ic.pre + "alternateWrapper") !== null)
  ...
  parseMmdbDataPart1(data, type) {
    ...
    for (let i in molid2rescount) {
      ...
      let chainName =
        molid2rescount[i].chain === undefined
          ? ""
          : molid2rescount[i].chain.trim();
      ...
      let chainNameFinal =
        chainNameHash[chainName] === 1
          ? chainName
          : chainName + chainNameHash[chainName].toString();
      ...
      ic.chainsColor[chain] =
        type !== undefined && !me.cfg.mmdbafid
          ? me.parasCls.thr(me.htmlCls.GREY8)
          : me.parasCls.thr(color);

      let geneId =
        molid2rescount[i].geneId === undefined ? "" : molid2rescount[i].geneId;
      let geneSymbol =
        molid2rescount[i].geneSymbol === undefined
          ? ""
          : molid2rescount[i].geneSymbol;
      let geneDesc =
        molid2rescount[i].geneDesc === undefined
          ? ""
          : molid2rescount[i].geneDesc;
      ic.chainsGene[chain] = {
        geneId: geneId,
  ...
  loadMmdbPrms(mmdbid, bGi, bCalpha) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let thisClass = this;
