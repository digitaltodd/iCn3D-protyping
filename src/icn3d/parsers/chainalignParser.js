...
class ChainalignParser {
  ...
  async downloadChainalignmentPart2(
    data1,
    data2Array,
    ...
    if (mmdbid_t.length > 5) {
      let bAppend = false,
        bNoDssp = true;
      hAtoms = await ic.pdbParserCls.loadPdbData(
        data1,
        mmdbid_t,
        false,
        bAppend,
      ...
      hAtoms = await ic.mmdbParserCls.parseMmdbData(
        data1,
        "target",
        chainidArray[0],
        0,
    ...
    for (let i = 0, il = data2Array.length; i < il; ++i) {
      ...
      mmdbid_q = chainidArray[i + 1].substr(
        0,
        chainidArray[i + 1].indexOf("_")
      );
      //mmdbid_q = (mmdbid_q_tmp.length == 5) ? mmdbid_q_tmp.substr(0, 4) : mmdbid_q_tmp; // added postfixfor same PDB IDs

      //if(mmdbid_q.length > 4) {
      if (mmdbid_q.length > 5) {
        // PDB ID plus postfix could be 5
        let bAppend = true,
          bNoDssp = true;
        hAtomsTmp = await ic.pdbParserCls.loadPdbData(
          data2Array[i],
          mmdbid_q,
          false,
          bAppend,
        ...
        let bNoSeqalign = true;
        hAtomsTmp = await ic.mmdbParserCls.parseMmdbData(
          data2Array[i],
          "query",
          chainidArray[i + 1],
          i,
    ...
    if (me.cfg.resnum) {
      ...
      for (let index in ic.afChainIndexHash) {
        ...
        if (me.cfg.aligntool != "tmalign") {
          let jsonStr_q = ic.domain3dCls.getDomainJsonForAlign(
            ic.chains[mmdbid_q + "_" + chain_q]
          );
          let jsonStr_t = ic.domain3dCls.getDomainJsonForAlign(
            ic.chains[mmdbid_t + "_" + chain_t]
          );
          ...
          let pdb_target = ic.saveFileCls.getAtomPDB(
            ic.chains[mmdbid_t + "_" + chain_t]
          );
      ...
      await thisClass.downloadChainalignmentPart2b(
        chainresiCalphaHash2,
        chainidArray,
        hAtoms,
        dataArray,
  ...
  async downloadChainalignmentPart2b(
    chainresiCalphaHash2,
    chainidArray,
    hAtoms,
    dataArray,
    ...
    // do the rest
    await this.downloadChainalignmentPart3(
      chainresiCalphaHash2,
      chainidArray,
      ic.hAtoms
    );
  ...
  setMsa(chainidArray, bVastplus, bRealign) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    ...
    for (let index = 1, indexl = chainidArray.length; index < indexl; ++index) {
      ...
      if (ic.qt_start_end && ic.qt_start_end[index - 1]) {
        for (let i = 0, il = ic.qt_start_end[index - 1].length; i < il; ++i) {
          alignLen +=
            parseInt(ic.qt_start_end[index - 1][i].q_end) -
            parseInt(ic.qt_start_end[index - 1][i].q_start) +
            1;
        }
      }
    ...
    let hAtomsAll = ic.setSeqAlignCls.setSeqAlignChainForAll(
      chainidArray,
      index_alignLen,
      bRealign
    );
  ...
  downloadChainalignmentPart2bRealign(dataArray, chainidPairArray, bReverse) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    ...
    for (let i = 0, il = dataArray.length; i < il; ++i) {
      ...
      if (mmdbidpairHash.hasOwnProperty(mmdbid1 + "_" + mmdbid2)) {
        // aligned already
        continue;
      }

      let bNoAlert = true;
      let bAligned = this.processAlign(
        align,
        i,
        queryData,
        bEqualMmdbid,
      ...
      if (bAligned) {
        ...
        mmdbid2cnt[mmdbid1] =
          mmdbid2cnt[mmdbid1] === undefined ? 1 : ++mmdbid2cnt[mmdbid1];
        mmdbid2cnt[mmdbid2] =
          mmdbid2cnt[mmdbid2] === undefined ? 1 : ++mmdbid2cnt[mmdbid2];
    ...
    for (let mmdbidpair in mmdbidpairFinalHash) {
      if (ic.q_rotation !== undefined) {
        ...
        let hAtomsTmp = ic.setSeqAlignCls.setSeqAlignChain(
          undefined,
          undefined,
          chainidArray
        );
  ...
  transformStructure(mmdbid, index, alignType, bForce) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    ...
    for (let i = 0, il = chainidArray.length; i < il; ++i) {
      for (let serial in ic.chains[chainidArray[i]]) {
        ...
        if (
          ic.q_rotation !== undefined &&
          (bForce || (!me.cfg.resnum && !me.cfg.resdef))
        ) {
  ...
  transformAtom(atm, index, alignType) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    if (alignType === "target") {
      ...
      let x =
        atm.coord.x * ic.q_rotation[index].x1 +
        atm.coord.y * ic.q_rotation[index].y1 +
        atm.coord.z * ic.q_rotation[index].z1;
      let y =
        atm.coord.x * ic.q_rotation[index].x2 +
        atm.coord.y * ic.q_rotation[index].y2 +
        atm.coord.z * ic.q_rotation[index].z2;
      let z =
        atm.coord.x * ic.q_rotation[index].x3 +
        atm.coord.y * ic.q_rotation[index].y3 +
        atm.coord.z * ic.q_rotation[index].z3;
  ...
  async downloadChainalignmentPart3(
    chainresiCalphaHash2,
    chainidArray,
    hAtoms
  ) {
    ...
    if (chainresiCalphaHash2 !== undefined)
      ic.ParserUtilsCls.transformToOpmOriForAlign(
        ic.selectedPdbid,
        chainresiCalphaHash2,
        true
      );
    ...
    if (me.cfg.rotate !== undefined)
      ic.resizeCanvasCls.rotStruc(me.cfg.rotate, true);
  ...
  addPostfixForChainids(chainidArray) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let struct2cnt = {};
    for (let i = 0, il = chainidArray.length; i < il; ++i) {
  ...
  async downloadChainalignment(chainalign, resnum, resdef) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let thisClass = this;
    ...
    if (ic.mmdbid_t.length > 5) {
      ...
      url_t =
        me.htmlCls.baseUrl +
        "mmdb/mmdb_strview.cgi?v=2&program=icn3d&b=1&s=1&ft=1&bu=" +
        me.cfg.bu +
        "&uid=" +
        ic.mmdbid_t;
    ...
    for (
      let index = 1, indexLen = alignArray.length;
      index < indexLen;
      ++index
    ) {
      ...
      ic.mmdbid_q =
        mmdbid_q_tmp.length == 5 ? mmdbid_q_tmp.substr(0, 4) : mmdbid_q_tmp; // added postfix for same PDB IDs
    ...
    for (
      ...
      if (!me.cfg.resnum && !me.cfg.resdef) {
        let chainalignFinal =
          ic.mmdbid_q + "_" + ic.chain_q + "," + ic.mmdbid_t + "_" + ic.chain_t;
        let domainalign =
          domainArray.length > 0
            ? domainArray[index] + "," + domainArray[0]
            : undefined;

        // TM-align (me.cfg.aligntool == 'tmalign') needs to input PDB
        if (
          me.cfg.aligntool != "tmalign" &&
          ic.mmdbid_t.length == 4 &&
          ic.mmdbid_q.length == 4
        ) {
          let urlalign;
          ...
          if (domainArray.length > 0) {
            urlalign =
              me.htmlCls.baseUrl +
              "vastdyn/vastdyn.cgi?domainpairs=" +
              domainalign;
          } else {
            urlalign =
              me.htmlCls.baseUrl +
              "vastdyn/vastdyn.cgi?chainpairs=" +
              chainalignFinal;
          }
    ...
    // try {
    let dataArray = await allPromise;
    await thisClass.parseChainAlignData(
      dataArray,
      alignArray,
      ic.mmdbid_t,
      ic.chain_t
  ...
  async parseChainAlignData(dataArray, chainidArray, mmdbid_t, chain_t) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let thisClass = this;
    ...
    let header =
      ...
      mmdbid_t +
      "\n";
    if (isNaN(mmdbid_t) && mmdbid_t.length > 5)
      targetData = header + targetData;
    ...
    for (let index = 1, indexl = chainidArray.length; index < indexl; ++index) {
      ...
      let header =
        ...
        mmdbid_q +
        "\n";
      if (isNaN(mmdbid_q) && mmdbid_q.length > 5)
        queryData = header + queryData;
      ...
      if (
        ...
        alert(
          "The coordinate data can NOT be retrieved for the structure " +
            mmdbid_q +
            "..."
        );
        return;
    ...
    await this.loadOpmDataForChainalign(
      targetData,
      queryDataArray,
      chainidArray,
      ic.mmdbidArray
  ...
  processAlign(align, index, queryData, bEqualMmdbid, bEqualChain, bNoAlert) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let bAligned = false;
    ...
    if (
      ...
      if (
        (align === undefined || align.length == 0) &&
        bEqualMmdbid &&
        bEqualChain
      ) {
        ...
        if (!me.cfg.command && !bNoAlert)
          alert(
            "These two chains can not align to each other. " +
        ...
        if (me.cfg.aligntool == "tmalign")
          logStr += "; TM-score: " + align[0].score.toPrecision(4);
        me.htmlCls.clickMenuCls.setLogCmd(logStr, false);
        let html =
          "<br><b>Alignment RMSD</b>: " + rmsd.toPrecision(4) + " &#8491;<br>";
        if (me.cfg.aligntool == "tmalign")
          html +=
            "<b>TM-score</b>: " + align[0].score.toPrecision(4) + "<br><br>";
        $("#" + ic.pre + "dl_rmsd_html").html(html);
        if (!me.cfg.bSidebyside)
          me.htmlCls.dialogCls.openDlg("dl_rmsd", "RMSD of alignment");
  ...
  async loadOpmDataForChainalign(data1, data2, chainidArray, mmdbidArray) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let thisClass = this;
    ...
    if (me.cfg.resnum || me.cfg.resdef) {
      ...
      await this.downloadChainalignmentPart2(
        data1,
        data2,
        undefined,
        chainidArray
      ...
      let url =
        me.htmlCls.baseUrl +
        "vastdyn/vastdyn.cgi?mmdbids2opm=" +
        mmdbidArray.join("','");
      ...
      if (!mmdbid) {
        if (!ic.bCommandLoad) ic.init(); // remove all previously loaded data
        await thisClass.downloadChainalignmentPart2(
          data1,
          data2,
          undefined,
          chainidArray
        ...
        ic.bOpm = true;
        let bVector = true;
        let chainresiCalphaHash = ic.loadPDBCls.loadPDB(
          opmdata,
          mmdbid,
          ic.bOpm,
          bVector
  ...
  async downloadMmdbAf(idlist, bQuery, vastplusAtype) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let thisClass = this;
    ...
    try {
      let dataArray = await allPromise;
      await thisClass.parseMMdbAfData(
        dataArray,
        structArray,
        bQuery,
        vastplusAtype
  ...
  async parseMMdbAfData(dataArray, structArray, bQuery, vastplusAtype) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let thisClass = this;
    ...
    for (let index = 0, indexl = structArray.length; index < indexl; ++index) {
      ...
      let header =
        ...
        structArray[index] +
        "\n";
      if (isNaN(structArray[index]) && structArray[index].length > 5)
        queryData = header + queryData;
    ...
    ic.opts["color"] =
      ic.structArray.length > 1
        ? "structure"
        : structArray[0].length > 5
        ? "confidence"
        : "chain";
    ...
    for (let i = 0, il = structArray.length; i < il; ++i) {
      ...
      if (isNaN(structArray[i]) && structArray[i].length > 5) {
        // PDB ID plus postfix could be 5
        //let bNoDssp = true;
    ...
    if (bQuery && me.cfg.matchedchains) {
      ...
      ic.bAnnoShown = false;
      if (
        $("#" + me.pre + "dl_selectannotations").hasClass(
          "ui-dialog-content"
        ) &&
        $("#" + ic.pre + "dl_selectannotations").dialog("isOpen")
      ) {
