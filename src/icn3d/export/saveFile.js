...
*/
  ...
  getAtomPDB(atomHash, bPqr, bNoChem, bNoHeader, chainResi2pdb, pdbid) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let pdbStr = "";
    ...
    for (let i in calphaHash) {
        ...
        if (atom.ss == "helix") {
          ...
          if (bHelixEnd)
            stru2header[stru] +=
              ...
              atom.chain.replace(/_/gi, "").substr(0, 2).padStart(2, " ") +
      ...
      if (atom.ssend) {
        if (atom.ss == "helix") {
          ...
          let helixLen =
            residEnd && residStart
              ? parseInt(residEnd.substr(residEnd.lastIndexOf("_") + 1)) -
                parseInt(residStart.substr(residStart.lastIndexOf("_") + 1))
              : 0;
          let helixType = 1;
          if (bHelixBegin)
            stru2header[stru] +=
  ...
  showTitle() {
    ...
    if (ic.molTitle !== undefined && ic.molTitle !== "") {
      ...
      if (ic.inputid === undefined) {
        ...
        console.log(ic.hAtoms);
        let structureArray = Object.keys(ic.utilCls.getStructures(ic.hAtoms)); //me.cfg.mmdbafid.split(',');
        if (structureArray.length > 1) {
          ...
          //let url = this.getLinkToStructureSummary();
          let url =
            isNaN(ic.inputid) && ic.inputid.length > 5
              ? "https://alphafold.ebi.ac.uk/entry/" + ic.inputid
              : "https://www.ncbi.nlm.nih.gov/structure/?term=" + ic.inputid;
